...
@section('content')
    <div class="w-full flex flex-wrap">
        ...
        <form action="{{ route('product.store', $title) }}" method="POST" enctype="multipart/form-data" class="w-full">
            @csrf
            <div class="fle flex-col justify-center">
                ...
                <div class="flex flex-col gap-1 mb-3">
                    ...
                    <select name="maskapai" id="maskapai" class="block mt-1 w-full rounded-md" required>
                ...
                <div class="flex flex-col gap-1 mb-3">
                    ...
                    <x-text-input id="price_quad" class="block mt-1 w-full" type="number" name="price_quad"
                        :value="old('price_quad')" placeholder="Price Quad" step="0.01" min='1' required autofocus />
                    @error('price_quad')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                <div class="flex flex-col gap-1 mb-3">
                    <x-input-label for="hotel_makkah" value="Hotel Makkah" />
                    <select name="hotel_makkah" id="hotel_makkah" class="block mt-1 w-full rounded-md
                    "
                        required>
                        <option value="" selected disabled>-- pilih hotel makkah --</option>
                        @foreach ($hotels_makkah as $hotel_makkah)
                            <option value="{{ $hotel_makkah->id }}">
                                {{ $hotel_makkah->nama }}
                                (⭐{{ $hotel_makkah->rating }})
                            </option>
                        @endforeach
                ...
                <div class="flex flex-col gap-1 mb-3">
                    <x-input-label for="hotel_madinah" value="Hotel Madinah" />
                    <select name="hotel_madinah" id="hotel_madinah" class="block mt-1 w-full rounded-md" required>
                        <option value="" selected disabled>-- pilih hotel madinah --</option>
                        @foreach ($hotels_madinah as $hotel_madinah)
                            <option value="{{ $hotel_madinah->id }}">{{ $hotel_madinah->nama }} {{ $hotel_makkah->nama }}
                                (⭐{{ $hotel_madinah->rating }})
                            </option>
                        @endforeach
                    </select>
                    @error('hotel_madinah')
                ...
                @if ($is_plus == 'yes')
                    <div class="flex flex-col gap-1 mb-3">
                        ...
                        <select name="rating_hotel_plus" id="rating_hotel_plus" class="block mt-1 w-full rounded-md"
                            required>
                            <option value="" selected disabled>pilih rating htel plus</option>
                ...
                <div class="flex flex-col gap-1 mb-3">
                    ...
                    <x-text-input id="kuota" class="block mt-1 w-full" type="number" name="kuota"
                        :value="old('kuota')" placeholder="Kuota" min='1' required autofocus />
                    @error('kuota')
                        <div class="text-red-500">{{ $message }}</div>      
                    @enderror
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-5">
                        ...
                        <ul id="list-free" class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                            ...
                            <li>
                              <div class="flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                <input id="checkbox-item-{{ $free->id }}-free" type="checkbox" name="free[]" value="{{ $free->id }}" @if(in_array($free->id, old('free', []))) checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                    ...
                    <div class="rounded-lg shadow w-72 max-h-96 mb-3">
                        ...
                        <ul id="list-promo" class="h-48 px-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                            ...
                            <li>
                              <div class="flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                                <input id="checkbox-item-{{ $promo->id }}-promo" type="checkbox" value="{{ $promo->id }}" name="promo[]" @if(in_array($promo->id, old('promo', []))) checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
